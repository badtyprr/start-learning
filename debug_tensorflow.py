...
def check():
    ...
    print("SSE       : %s" % cpu_is_set(1, 3, 25))
    print("SSE2      : %s" % cpu_is_set(1, 3, 26))
    print("SSE3      : %s" % cpu_is_set(1, 2, 0))
    print("SSSE3     : %s" % cpu_is_set(1, 2, 9))
    print("SSE4.1    : %s" % cpu_is_set(1, 2, 19))
    print("SSE4.2    : %s" % cpu_is_set(1, 2, 20))
    print("SSE4a     : %s" % cpu_is_set(0x80000001, 2, 6))
    print("AVX       : %s" % cpu_is_set(1, 2, 28))
    print("AVX2      : %s" % cpu_is_set(7, 1, 5))
    print("BMI1      : %s" % cpu_is_set(7, 1, 3))
    print("BMI2      : %s" % cpu_is_set(7, 1, 8))
...
install it using the command `pip install tensorflow`.""")
    ...
    # Check for Windows C runtime library
    try:
...
- Could not load 'msvcp140.dll'. You may install this DLL by downloading Microsoft Visual
  ...
  https://www.microsoft.com/en-us/download/details.aspx?id=53587""")
    ...
    try:
        ...
        try:
            # CUDA 10.0
            cudart64_100 = ctypes.WinDLL("cudart64_100.dll")
            cudart64_100_path = find_library("cudart64_100.dll")
            print('- Cuda 10.0 found at {}'.format(cudart64_100_path))
        except OSError:
            candidate_explanation = True
            print("""- Could not load 'cudart64_###.dll'.""")
            print("""Download and install the appropriate CUDA from
                this URL: https://developer.nvidia.com/cuda-toolkit""")
    # Not on Windows...
    except AttributeError:
        try:
            cudart64 = ctypes.cdll["libcudart.so"]
            cudart64_path = find_library("cudart")
            print('- Cuda found at {}'.format(cudart64_path))
            # TODO: Look for version.txt file in CUDA path
        except OSError:
            candidate_explanation = True
            print("""- Could not load 'cudart64_###.dll'.""")
  ...
  correct driver installed.""")
    # Not in Windows...
        ...
        try:
            ...
            nvcuda_path = find_library("cuda")
            print('- libcuda.so found at {}'.format(nvcuda_path))
        except OSError:
            candidate_explanation = True
            print("""
        - Could not load 'nvcuda.dll'. The GPU version of TensorFlow requires that
...
- Could not load 'cudnn64_7.dll'. The GPU version of TensorFlow
  ...
  https://developer.nvidia.com/cudnn""")
        ...
        try:
            cudnn7 = ctypes.cdll["libcudnn.so.7"]
            cudnn7_path = find_library("cudnn")
            print("- cuDNN Found at {}".format(cudnn7_path))
            cudnn7_found = True
        except OSError:
            candidate_explanation = True
